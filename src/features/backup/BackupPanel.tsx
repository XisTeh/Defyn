...
export function BackupPanel({ onRestored, onReset }: { onRestored: () => Promise<void>; onReset: () => Promise<void> }) {
  ...
  return <div className="backup-page page-container" data-pwa-update-blocking={candidate || restoring || resetOpen || resetting ? 'true' : undefined}>
    ...
    <section className="local-reset-card" aria-labelledby="local-reset-title"><div><span className="page-eyebrow">Dados locais</span><h2 id="local-reset-title">Começar do zero neste dispositivo</h2><p>Apaga os perfis e registros armazenados neste navegador. Esta ação não pode ser desfeita sem um backup.</p></div><Button variant="danger" compact type="button" onClick={() => setResetOpen(true)}>Resetar DEFYN neste dispositivo</Button></section>
    ...
    {resetOpen && <ResetDialog busy={resetting} onClose={() => { if (!resetting) setResetOpen(false); }} onExport={() => void exportBackup(false)} onReset={() => void resetLocalData()} />}
  </div>;
}

function ResetDialog({ busy, onClose, onExport, onReset }: { busy: boolean; onClose: () => void; onExport: () => void; onReset: () => void }) {
  const dialogRef = useRef<HTMLElement>(null);
  const closeRef = useRef<HTMLButtonElement>(null);
  const [confirmation, setConfirmation] = useState('');
  useAccessibleDialog(dialogRef, onClose, closeRef);
  const confirmed = confirmation === 'RESETAR';
  return <div className="local-reset-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><section ref={dialogRef} className="local-reset-dialog" role="dialog" aria-modal="true" aria-labelledby="local-reset-dialog-title" aria-describedby="local-reset-dialog-description"><header><div><span className="page-eyebrow">Ação irreversível</span><h2 id="local-reset-dialog-title">Apagar dados deste dispositivo</h2></div><button ref={closeRef} type="button" onClick={onClose} disabled={busy} aria-label="Fechar">×</button></header><p id="local-reset-dialog-description">Perfis, metas, água, resumo diário, rotina, sono, treinos, progresso, fotos, mídia, preferências por perfil e dados legados deste navegador serão apagados. O DEFYN continuará instalado e abrirá como uma instalação nova.</p><p className="local-reset-warning">Se quiser guardar uma cópia, exporte um backup antes de continuar.</p><label>Digite <strong>RESETAR</strong> para habilitar a ação<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" autoCapitalize="characters" spellCheck={false} aria-describedby="local-reset-dialog-description" disabled={busy}/></label><footer><Button variant="secondary" type="button" onClick={onExport} disabled={busy}>Exportar backup</Button><Button variant="danger" type="button" loading={busy} disabled={!confirmed} onClick={onReset}>Apagar todos os dados</Button></footer></section></div>;
}
