...
describe('sessão de treino', () => {
  ...
  it('persiste rascunho de série sem iniciar descanso e o conclui no mesmo registro', async () => { const { service, sessions } = setup(); const session = await service.startSession('a', 'plan-a', 'template-a'); const draft = await service.logSet('a', session.id, 'supino', 0, { actualLoad: 30, actualReps: 9, completed: false }); expect(draft.session.restEndsAt).toBeUndefined(); expect(draft.log.completed).toBe(false); const done = await service.logSet('a', session.id, 'supino', 0, { actualLoad: 32.5, actualReps: 10, completed: true }); expect(done.log.id).toBe(draft.log.id); expect((await sessions.listSetLogs('a', session.id))).toHaveLength(1); });
  it('não cria sessão duplicada quando já existe uma ativa', async () => { const { service, sessions } = setup(); const first = await service.startSession('a', 'plan-a', 'template-a'); const second = await service.startSession('a', 'plan-a', 'template-a'); expect(second.id).toBe(first.id); expect(await sessions.list('a')).toHaveLength(1); });
  ...
  it('reordena exercícios só na sessão e mantém o exercício atual selecionado', async () => { const { service, sessions, plans } = setup(); const original = await service.startSession('a', 'plan-a', 'template-a'); const extended = { ...original, exercises: [...original.exercises, { ...original.exercises[0]!, prescriptionId: 'rx-2', exerciseId: 'halteres', name: 'Supino com halteres' }] }; await sessions.save(extended); const changed = await service.reorderSessionExercises('a', original.id, 0, 1); expect(changed.exercises.map((item) => item.exerciseId)).toEqual(['halteres', 'supino']); expect(changed.currentExerciseIndex).toBe(1); expect((await plans.getById('a', 'plan-a'))?.versions[0]?.templates[0]?.exercises[0]?.exerciseId).toBe('supino'); });
  ...
  it('expõe a sessão concluída no estado de Hoje', async () => { const { service } = setup(); const session = await service.startSession('a', 'plan-a', 'template-a'); await service.finish('a', session.id); expect((await service.getToday('a')).completedSession?.id).toBe(session.id); });
  it('cancela sem apagar séries e sem contar como concluída', async () => { const { service, sessions } = setup(); const session = await service.startSession('a', 'plan-a', 'template-a'); await service.logSet('a', session.id, 'supino', 0, { actualLoad: 30, actualReps: 10, completed: true }); const cancelled = await service.cancel('a', session.id); expect(cancelled.status).toBe('cancelled'); expect(await sessions.listSetLogs('a', session.id)).toHaveLength(1); expect((await sessions.list('a')).filter((item) => item.status === 'completed')).toHaveLength(0); });
