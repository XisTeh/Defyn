...
export class ProgressService {
  ...
  async savePhoto(profileId:string,checkInId:string|undefined,localDate:string,category:ProgressPhotoCategory,prepared:LocalMedia,note?:string){ const timestamp=this.now().toISOString(); const photo:ProgressPhotoMetadata={id:prepared.ownerId??this.id(),profileId,localDate,occurredAt:`${localDate}T12:00:00`,category,mediaId:prepared.id,checkInId,note:note?.trim()||undefined,createdAt:timestamp,updatedAt:timestamp}; const media={...prepared,profileId,ownerType:'progress' as const,ownerId:photo.id}; if(this.progress.savePhotoWithMedia) await this.progress.savePhotoWithMedia(photo,media); else { await this.media.save(media,profileId); await this.progress.savePhotoMetadata(photo); } return photo; }
