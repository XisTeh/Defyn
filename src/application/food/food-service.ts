...
export interface SaveFoodInput {
  ...
  nutrients: NutrientValues; portions?: FoodPortion[]; dataSource: Food['dataSource']; nutritionLabelPhotoRef?: string; allergens?: string[]; tags?: string[];
}
...
export class FoodService {
  ...
  async save(input: SaveFoodInput): Promise<{ food: Food; duplicates: Food[] }> {
    ...
    const food: Food = {
      ...
      portions: input.portions ?? [], nutrients: input.nutrients, dataSource: input.dataSource, nutritionLabelPhotoRef: input.nutritionLabelPhotoRef,
