...
describe('migration v3 → v4', () => {
  ...
  it('mantém catálogo base fora da migration e com ids estáveis', () => { expect(BASE_EXERCISES[0]?.id).toBe('defyn-exercise-01'); expect(BASE_EXERCISES[51]?.id).toBe('defyn-exercise-52'); expect(BASE_EXERCISES.length).toBeGreaterThan(250); });
});
