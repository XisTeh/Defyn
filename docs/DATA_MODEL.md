...
A cadeia v7–v8 continua preservada. A migration v9 mantém todas as stores e normaliza a PK técnica de `RoutineDay` para UUID, remapeando referências de sync atomicamente. A identidade funcional permanece no índice único `profileId + dayOfWeek`.
...
`routine_days.id` é um UUID técnico estável. O dia da semana não é codificado no ID: a identidade funcional e a proteção contra duplicação usam `account_id + profile_id + day_of_week`, já garantidos por UNIQUE no schema remoto.
