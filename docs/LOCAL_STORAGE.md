...
O banco `defyn-local` usa IndexedDB por Dexie, atualmente no schema v8. Todas as entidades pessoais carregam `profileId` e as consultas de interface filtram esse proprietário.
...
A 1.1.0 não altera novamente o schema IndexedDB: `media`, outbox, metadata, cursores e conflitos existentes já comportam o cache e o pipeline. Portanto a versão final permanece v8, sem reset ou migration adicional.
