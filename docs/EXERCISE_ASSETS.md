...
- O manifesto `exercise-media.ts` preserva os metadados das 52 ilustrações originais; `exercise-images.ts` carrega em build-time o PNG exclusivo de cada um dos 242 exercícios publicados.
...
- As 242 miniaturas PNG entram no precache estático; mídia de exercício personalizada continua pessoal no IndexedDB e não é adicionada ao service worker.
